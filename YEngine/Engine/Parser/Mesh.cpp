...
#include <Parser/Bindable/Vertex.h>
...
UINT Model::countModels = 0;

ModelException::ModelException(int line, const char* file, std::string note) noexcept
	:
	YException(line, file),
	note(std::move(note))
{}
...
std::unique_ptr<Mesh> Model::ParseMesh( Graphics& gfx, const aiMesh& mesh,const aiMaterial* const* pMaterials,const std::filesystem::path& path,float scale )
{
	...
	if (hasDiffuseMap && hasNormalMap && hasSpecularMap)
	{
		...
		auto pvs = VertexShader::Resolve(gfx, "Engine\\assets\\shaders\\PhongVSNormalMap.hlsl");
		auto pvsbc = pvs->GetBytecode();
		bindablePtrs.push_back(std::move(pvs));

		bindablePtrs.push_back( PixelShader::Resolve( gfx,
			hasAlphaDiffuse ? "Engine\\assets\\shaders\\PhongPSSpecNormMask.hlsl" : "Engine\\assets\\shaders\\PhongPSSpecNormalMap.hlsl"
		) );
	...
	else if (hasDiffuseMap && hasNormalMap)
	{
		...
		bindablePtrs.push_back(PixelShader::Resolve(gfx, "Engine\\assets\\shaders\\PhongPSNormalMap.hlsl"));
	...
	else if( hasDiffuseMap && !hasNormalMap && hasSpecularMap )
	{
		...
		bindablePtrs.push_back( PixelShader::Resolve( gfx,"Engine\\assets\\shaders\\PhongPSSpec.hlsl" ) );
	...
	else if (hasDiffuseMap)
	{
		...
		bindablePtrs.push_back(PixelShader::Resolve(gfx, "Engine\\assets\\shaders\\PhongPS.hlsl"));
	...
	else if (!hasDiffuseMap && !hasNormalMap && !hasSpecularMap)
	{
		...
		auto pvs = VertexShader::Resolve(gfx, "Engine\\assets\\shaders\\PhongVSNotex.hlsl");
		auto pvsbc = pvs->GetBytecode();
		bindablePtrs.push_back(std::move(pvs));

		bindablePtrs.push_back(PixelShader::Resolve(gfx, "Engine\\assets\\shaders\\PhongPSNotex.hlsl"));
