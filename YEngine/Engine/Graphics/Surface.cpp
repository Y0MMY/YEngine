...
Surface Surface::FromFile( const std::string& name )
{
	...
	HRESULT hr = DirectX::LoadFromWICFile( FileSystem::StringToWide( name ).c_str(), DirectX::WIC_FLAGS_IGNORE_SRGB, nullptr, scratch);
...
void Surface::Save( const std::string& filename ) const
{
	...
	HRESULT hr = DirectX::SaveToWICFile(
		*scratch.GetImage( 0,0,0 ),
		...
		FileSystem::StringToWide( filename ).c_str()
	);
	if( FAILED( hr ) )
	{
...
const char* Surface::Exception::what() const noexcept
{
	...
	oss << YException::what() << std::endl << GetNote();
	whatBuffer = oss.str();
